...
第5回では、`postfile` を使って回帰結果をExcelへ出力する方法を学びました。あの方法はとても実務的で、成果物を残すという意味で大事でした。ただし、1つ弱点がありました。**コードが長い**ことです。
...
第5回のコードでは、各モデルのたびに次のような処理を繰り返していました。
...
3. どの部分が短くなったか、自分の言葉で説明する  
4. `collect_result.ado` を使わずにベタ書きした版と見比べて、どちらが読みやすいか考える  
